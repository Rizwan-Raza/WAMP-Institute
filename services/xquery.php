...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    if (isset($_POST['fname']) and isset($_POST['lname']) and isset($_POST['email']) and isset($_POST['number']) and isset($_POST['shift']) and isset($_POST['number'])) {
        error_reporting(0);
        extract($_POST, EXTR_SKIP);

        // $sql = "INSERT INTO `queries`(`name`, `email`, `number`, `type`) VALUES ('$name','$email','$number', 1)";

        $sql = "INSERT INTO `xqueries`(`gender`, `firstname`, `lastname`, `reference`, `shift`, `email`, `number`, `time`) VALUES ($gender, '$fname','$lname','" . implode(",", $reference) . "', $shift, '$email','$number', CONVERT_TZ(CURRENT_TIMESTAMP, '-07:00', '+05:30'))";
        ...
        <body style="font-family: Arial, Helvetica, sans-serif;margin:0px;background-color: #ffffff;">
            <table style="background-color: #eeeeee;padding: 8px 16px;width: 100%;box-shadow: 0 2px 2px 0 rgba(0,0,0,0.14), 0 3px 1px -2px rgba(0,0,0,0.12), 0 1px 5px 0 rgba(0,0,0,0.2);">
                <tr>
                    ...
                    <td style="line-height: 50px;vertical-align: top; margin:0px; font-size: 32px; font-weight: 500;">Enroll Request from WAMP D.M. Institute Landing Page #1</td>
                </tr>
            </table>
